...
public class BDUser extends BDService {
    ...
    public boolean EditarU(String user, String nombre, String edad, String mail, String usuario, String contra) {
        boolean correcto = false;
        BDService bdService = new BDService(context);
        SQLiteDatabase db = bdService.getWritableDatabase();
        try {
            db.execSQL("UPDATE " + TABLE_ESPE + " SET usuario = '"+ usuario +"', contra = '" + contra + "', nombre = '"+ nombre +"', email = '"+ mail +"', edad = '"+ edad +"' WHERE usuario ='" + user + "'");
            correcto = true;
        } catch (Exception ex) {
            ex.toString();
        }
